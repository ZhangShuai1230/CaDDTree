...
def load_and_process_dataset(data_name: str):
    if data_name == "gsm8k":
        ...
        urls = [base + f for f in ["test.jsonl", "test2.jsonl", "test3.jsonl",
...
class Qwen3DFlashAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        k = self.k_norm(torch.cat([k_ctx, k_noise], dim=1).view(bsz, ctx_len + q_len, -1, self.head_dim)).transpose(1, 2)
        v = torch.cat([v_ctx, v_noise], dim=1).view(bsz, ctx_len + q_len, -1, self.head_dim).transpose(1, 2)
        cos, sin = position_embeddings
        q, k = apply_rotary_pos_emb(q, k, cos, sin)
        if past_key_values is not None:
            k, v = past_key_values.update(k, v, self.layer_idx, {"sin": sin, "cos": cos, "cache_position": cache_position})
        attn_fn: Callable = eager_attention_forward if self.config._attn_implementation == "eager" else ALL_ATTENTION_FUNCTIONS[self.config._attn_implementation]
