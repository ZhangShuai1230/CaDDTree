...
from dflash import dflash_generate, cuda_time
...
def load_cpp_compact_module():
    try:
        ...
        print(f"torch.utils.cpp_extension is unavailable; falling back to Python cache compaction. {exc}")
        return None
...
"""
    try:
        ...
        print("Loaded inline C++ tail cache compaction extension for DDTree.")
        return module
    except Exception as exc:
        print(f"Failed to build inline C++ tail cache compaction extension; falling back to Python implementation. {exc}")
        return None
...
def ddtree_generate(
    ...
    while start < max_length:
        ...
        node_token_ids, node_depths, parents, child_maps, visibility_cpu = build_ddtree_tree(
